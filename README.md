...
适用于 [iSH](https://ish.app/) 的自定义 Alpine rootfs 构建与发布。
...
- 预装：`alpine-base` + `alpine-conf`（含 `setup-alpine`）、`nano`、`git`、`curl`/`wget`、`openssh-client`、`ca-certificates`、`tzdata`、`coreutils` 等常用工具。完整清单见 [`build/packages.txt`](build/packages.txt)。
- 不启用任何系统服务（iSH 无真实 init）。
...
iSH 支持从 URL 直接导入 rootfs（具体方式以 iSH 当前版本文档为准）。导入后 `setup-alpine`、`nano`、`git` 等命令开箱可用。
